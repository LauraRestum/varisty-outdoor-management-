import Link from 'next/link'
import { ArrowRight, Check, Snowflake, AlertTriangle } from 'lucide-react'
import type { Metadata } from 'next'

export const metadata: Metadata = {
  title: 'Snow Removal Service | 24/7 Winter Coverage | Varsity Outdoor',
  description:
    'Professional snow plowing, salting, and ice management for driveways and commercial lots. Priority 4-hour SLA available. Central Ohio winter coverage.',
}

const FEATURES = [
  '24/7 winter storm monitoring',
  'Plowing, salting & de-icing',
  'Sidewalk & stair clearing',
  'Commercial lot service available',
  'Priority 4-hour SLA option',
  'Seasonal or per-storm contracts',
]

export default function SnowRemovalPage() {
  return (
    <div className="bg-dark-bg min-h-screen">
      <section className="py-20 md:py-28 bg-dark-card border-b border-dark-border">
        <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8">
          <div className="flex items-center gap-3 mb-6">
            <Link href="/services" className="text-gray-500 hover:text-white text-sm transition-colors">Services</Link>
            <span className="text-gray-600">/</span>
            <span className="text-brand-green text-sm font-semibold">Snow Removal</span>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="w-16 h-16 rounded-2xl bg-blue-900/30 border border-blue-700/30 flex items-center justify-center mb-6">
                <Snowflake className="w-8 h-8 text-blue-400" />
              </div>
              <h1 className="font-condensed text-5xl md:text-6xl font-black uppercase text-white mb-4">
                NEVER SHOVEL <span className="text-brand-green">AGAIN</span>
              </h1>
              <p className="text-gray-400 text-xl leading-relaxed mb-6">
                Varsity&apos;s snow removal team is ready before the first flake falls. We monitor weather around the clock and mobilize the moment conditions require it.
              </p>

              {/* Priority SLA callout */}
              <div className="bg-blue-900/20 border border-blue-800 rounded-xl p-4 mb-6 flex items-start gap-3">
                <AlertTriangle className="w-5 h-5 text-blue-400 flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-blue-300 font-bold text-sm">Priority Service Available</p>
                  <p className="text-blue-400/80 text-sm">Guaranteed cleared within 4 hours of storm end — ideal for medical needs or early commuters.</p>
                </div>
              </div>

              <Link href="/quote" className="inline-flex items-center gap-2 bg-brand-green hover:bg-brand-green-dark text-black font-black px-8 py-4 rounded-xl transition-colors text-lg uppercase">
                Get Your Free Quote <ArrowRight className="w-5 h-5" />
              </Link>
            </div>
            <div className="bg-dark-bg rounded-2xl border border-dark-border p-6">
              <h3 className="font-condensed text-lg font-bold uppercase text-brand-green mb-4">What&apos;s Included</h3>
              <ul className="space-y-3">
                {FEATURES.map((f) => (
                  <li key={f} className="flex items-start gap-3">
                    <div className="w-5 h-5 rounded-full bg-brand-green/20 border border-brand-green/30 flex items-center justify-center flex-shrink-0 mt-0.5">
                      <Check className="w-3 h-3 text-brand-green" />
                    </div>
                    <span className="text-gray-300 text-sm">{f}</span>
                  </li>
                ))}
              </ul>
            </div>
          </div>
        </div>
      </section>

      <section className="py-16 bg-dark-card border-t border-dark-border">
        <div className="max-w-3xl mx-auto px-4 text-center">
          <h2 className="font-condensed text-4xl font-black uppercase text-white mb-4">
            Beat the <span className="text-brand-green">Winter Rush</span>
          </h2>
          <p className="text-gray-400 mb-8">Spots fill up fast once the season starts. Lock in your snow removal plan today.</p>
          <Link href="/quote" className="inline-flex items-center gap-2 bg-brand-green hover:bg-brand-green-dark text-black font-black px-10 py-5 rounded-xl transition-colors text-xl uppercase">
            Get My Snow Removal Quote <ArrowRight className="w-6 h-6" />
          </Link>
        </div>
      </section>
    </div>
  )
}
